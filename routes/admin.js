...
router.post('/channels', (req, res) => {
  try {
    const { name, category, stream_url, backup_url, backup_url2, logo_url, sort_order, watermark, watermark_url, watermark_position, watermark_opacity, watermark_size } = req.body;
    const result = run('INSERT INTO channels (name, category, stream_url, backup_url, backup_url2, logo_url, sort_order, watermark, watermark_url, watermark_position, watermark_opacity, watermark_size) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [name, category || 'general', stream_url || '', backup_url || '', backup_url2 || '', logo_url || '', sort_order || 0, watermark !== undefined ? watermark : 1, watermark_url || '', watermark_position || '', watermark_opacity || null, watermark_size || null]);
...
router.put('/channels/:id', (req, res) => {
  const { name, category, stream_url, backup_url, backup_url2, logo_url, is_active, sort_order, watermark, watermark_url, watermark_position, watermark_opacity, watermark_size } = req.body;
  const ch = getOne('SELECT * FROM channels WHERE id = ?', [req.params.id]);
  if (!ch) return res.status(404).json({ error: 'Channel not found' });
  run('UPDATE channels SET name=?, category=?, stream_url=?, backup_url=?, backup_url2=?, logo_url=?, is_active=?, sort_order=?, watermark=?, watermark_url=?, watermark_position=?, watermark_opacity=?, watermark_size=? WHERE id=?', [
    name ?? ch.name, category ?? ch.category, stream_url ?? ch.stream_url,
    backup_url ?? ch.backup_url, backup_url2 ?? ch.backup_url2,
    logo_url ?? ch.logo_url, is_active ?? ch.is_active, sort_order ?? ch.sort_order, watermark ?? ch.watermark,
    watermark_url ?? ch.watermark_url, watermark_position ?? ch.watermark_position, watermark_opacity ?? ch.watermark_opacity, watermark_size ?? ch.watermark_size, req.params.id
  ]);
...
router.post('/movies', (req, res) => {
  try {
    const { title, description, category, poster_url, video_url, duration, year, rating, cast_list, director, genre, backdrop_url, watermark, watermark_url, watermark_position, watermark_opacity, watermark_size } = req.body;
    const result = run('INSERT INTO movies (title, description, category, poster_url, video_url, duration, year, rating, cast_list, director, genre, backdrop_url, watermark, watermark_url, watermark_position, watermark_opacity, watermark_size) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [title, description, category || 'general', poster_url || '', video_url || '', duration, year, rating, cast_list || '', director || '', genre || '', backdrop_url || '', watermark !== undefined ? watermark : 1, watermark_url || '', watermark_position || '', watermark_opacity || null, watermark_size || null]);
...
router.put('/movies/:id', (req, res) => {
  const { title, description, category, poster_url, video_url, duration, year, rating, is_active, cast_list, director, genre, backdrop_url, watermark, watermark_url, watermark_position, watermark_opacity, watermark_size } = req.body;
  const mv = getOne('SELECT * FROM movies WHERE id = ?', [req.params.id]);
  if (!mv) return res.status(404).json({ error: 'Movie not found' });
  run('UPDATE movies SET title=?, description=?, category=?, poster_url=?, video_url=?, duration=?, year=?, rating=?, is_active=?, cast_list=?, director=?, genre=?, backdrop_url=?, watermark=?, watermark_url=?, watermark_position=?, watermark_opacity=?, watermark_size=? WHERE id=?', [
    ...
    backdrop_url ?? mv.backdrop_url, watermark ?? mv.watermark,
    watermark_url ?? mv.watermark_url, watermark_position ?? mv.watermark_position, watermark_opacity ?? mv.watermark_opacity, watermark_size ?? mv.watermark_size, req.params.id
  ]);
...
router.post('/series', (req, res) => {
  try {
    const { title, description, category, poster_url, total_seasons, year, rating, cast_list, director, genre, backdrop_url, watermark, watermark_url, watermark_position, watermark_opacity, watermark_size } = req.body;
    const result = run('INSERT INTO series (title, description, category, poster_url, total_seasons, year, rating, cast_list, director, genre, backdrop_url, watermark, watermark_url, watermark_position, watermark_opacity, watermark_size) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [title, description, category || 'drama', poster_url || '', total_seasons || 1, year, rating, cast_list || '', director || '', genre || '', backdrop_url || '', watermark !== undefined ? watermark : 1, watermark_url || '', watermark_position || '', watermark_opacity || null, watermark_size || null]);
...
router.put('/series/:id', (req, res) => {
  const { title, description, category, poster_url, total_seasons, year, rating, is_active, cast_list, director, genre, backdrop_url, watermark, watermark_url, watermark_position, watermark_opacity, watermark_size } = req.body;
  const s = getOne('SELECT * FROM series WHERE id = ?', [req.params.id]);
  if (!s) return res.status(404).json({ error: 'Series not found' });
  run('UPDATE series SET title=?, description=?, category=?, poster_url=?, total_seasons=?, year=?, rating=?, is_active=?, cast_list=?, director=?, genre=?, backdrop_url=?, watermark=?, watermark_url=?, watermark_position=?, watermark_opacity=?, watermark_size=? WHERE id=?', [
    ...
    backdrop_url ?? s.backdrop_url, watermark ?? s.watermark,
    watermark_url ?? s.watermark_url, watermark_position ?? s.watermark_position, watermark_opacity ?? s.watermark_opacity, watermark_size ?? s.watermark_size, req.params.id
  ]);
